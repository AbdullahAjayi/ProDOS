...
import { updateReminder as updateReminderSchedule } from "./reminderService.js";
...
async function updateReminderConversation(
    ...
    await ctx.reply("Which habit would you like to update the reminder for?", {
    ...
    const habitId = habitSelection.callbackQuery.data.replace("update_habit_", "");
    ...
    await ctx.reply(
        `✅ Reminder updated for <b>${selectedHabit.name}</b>!${newTime ? `\n⏰ New time: ${newTime}` : ""
        }${newDays ? `\n📅 Days: ${newDays.join(", ")}` : ""}`,
        {
            parse_mode: "HTML",
            reply_markup: { remove_keyboard: true },
...
export default updateReminderConversation;
